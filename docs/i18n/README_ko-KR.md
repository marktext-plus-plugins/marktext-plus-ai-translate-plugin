...
이 저장소는 의도적으로 MarkText Plus의 검증을 받지 않았습니다. 설치하기 전에 소스를 읽어 보세요 — 파일은 두 개뿐입니다.
...
두 파일입니다: [`plugin.lua`](../../plugin.lua) 과 [`lib/marktext-plus.lua`](../../lib/marktext-plus.lua) — 후자는 SDK의 API 모듈로 `require`로 불러옵니다 — 빌드 없음, 의존성 없음, Windows·macOS·Linux에서 동일합니다. 에디터 내부의 샌드박스에서 실행되며 파일 시스템도, 네트워크도, `os` 라이브러리도 없습니다.
