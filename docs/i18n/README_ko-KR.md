...
셋 중 무엇을 잊은 틀이든 그것을 버리지 않고 뒤에 붙입니다. 다뤄야 할 것이 빠진 프롬프트는 정돈되지 않은 것보다 나쁘기 때문입니다 — 모델은 어느 쪽이든 답하므로, 겉보기에는 아무 문제 없는 채로 아무도 묻지 않은 물음에 답하게 됩니다. 각 칸은 기본값을 보여 주므로 무엇을 바꾸는지 보입니다. 비우면 기본값이 돌아옵니다.
